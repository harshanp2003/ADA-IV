...
void gen_worst_data(int a[], int l, int r) //Worst Data generation
{
    if (l < r)
    {
        int m = l + (r - l) / 2;
...
int merge(int b[], int c[], int a[], int n, int m)  //Merge Function
{
    int i = 0, j = 0, k = 0, ct = 0;   //blen=n,  clen=m
    while (i < n && j < m)
    {
        if (b[i] < c[j])
        {
...
void mergeSort(int a[], int n, int *ct) //Merge sort function
{
    int i = 0, j = 0;
    if (n == 1)
        return;
