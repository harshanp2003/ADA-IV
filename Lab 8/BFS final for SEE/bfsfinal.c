...
struct q
{
    int *arr, f, r, c;//c=count
};
void init(struct q *qu, int n)
{
    qu->arr = malloc(20 * sizeof(int));
    qu->f = 0;
    qu->r = -1;
    qu->c = 0;
}
...
void genData(int ch) //ch=0 for worst,ch=1 for best case is connected and acyclic ,worst case is connected and cyclic
{
    printf("Graph is\n");
    for (int i = 0; i <n; i++)
    {
